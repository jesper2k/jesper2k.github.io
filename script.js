...
function instantiateProgramTable() {
    for (var i = 0; i < programs.length; i++) {
        var x =
            tableItemMargin / 2 -
            ((tableItemWidth + tableItemMargin) * tableItemsX) / 2 +
            (i % tableItemsX) * (tableItemWidth + tableItemMargin)
        var y =
            Math.floor(i / tableItemsX) * (tableItemHeight + tableItemMargin)
        ...
        var htmlstuff = ""
        htmlstuff +=
            "<div class='outerProgramDiv' onClick=redirectTo('" +
            address +
            "') "
        htmlstuff +=
            "style ='margin-left:" +
            ...
            "px; margin-top:" +
            y +
            "px;'><div class='innerProgramDiv' style='background-image: linear-gradient(160deg, " +
            colors.color1 +
            "," +
            colors.color2 +
            ");'>"
...
function colorGenerator(value) {
    var colors = {
        /* Default value */ color1: "rgb(100, 200, 250)",
